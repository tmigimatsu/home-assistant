...
class MultiChannelRelay(SwitchDevice):
    ...
    def turn_on(self, **kwargs):
        """Turn the switch on."""
        status = self._rfm69.send_message(self._node_id, "S.%d.1.%d." % (self._relay_pin, self._timeout))[0]
        if status is None:
    ...
    def turn_off(self, **kwargs):
        """Turn the switch off."""
        status = self._rfm69.send_message(self._node_id, "S.%d.0." % (self._relay_pin))[0]
        if status is None:
    ...
    def update(self):
        ...
        status = self._rfm69.send_message(self._node_id, "G.", cache=self._cache)[0]
        if status is None:
